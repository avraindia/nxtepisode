...
<div class="body">
    <div class="card">
        <div class="card-body">
            ...
            <input type="hidden" name="item_id" class="item_id" value="{{$collection_item->id}}">
            <input type="hidden" name="section_id" class="section_id" value="{{$collection_item->section_id}}">
        </div>
    </div>
</div>
...
$(document).on("change",".add_product_section_btn",function(e) {
    ...
    addSectionProduct({is_checked:is_checked, item_id:item_id, section_id:section_id, product_id:product_id}).then(res=>{
        console.log(res);
    });
});
...
const addSectionProduct = (options) => {
    ...
    return fetch("{{ route('add_homepage_collection_product') }}", requestOptions)
    .then(response => {
...
function get_filtering_body(page){
    ...
    $.ajax({
        url: "{{ route('filtering_collection_product') }}",
        method: 'POST',
        data: {_token: _token, page:page, product_cat_ids:product_cat_ids, option_value_ids:option_value_ids, fitting_type_id:fitting_type_id, product_theme_ids:product_theme_ids, product_gender_id:product_gender_id, item_id:item_id, section_id:section_id},
